...
public class HttpUtil {
  ...
  public static String post(String url, Header[] headers, String requestBody, String charset) {
    ...
    try {
      ...
      httpPost.setEntity(new StringEntity(String.valueOf(requestBody), charset));
      CloseableHttpResponse response = httpclient.execute(httpPost, context);
      try {
        String responseBody = EntityUtils.toString(response.getEntity(), charset);
        return responseBody;
  ...
  public static String login(String url, Header[] headers, String requestBody, String charset) {
    HttpPost httpPost = new HttpPost(url);
    httpPost.setConfig(getRequestConfig());
    httpPost.setHeaders(headers);
    ...
    try {
      ...
      try {
        ...
        new RuntimeException("로그인 요청 실패", e);
      }
    }
