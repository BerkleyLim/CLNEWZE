...
2) UI 구현 - 기본페이지 완료 (관리자, 프로필은 추후 추가예정)
3) DB 설계 - 기본 부분 완료, jwt auth와 관리자용 디비 회원 정보 디비는 설계예젗
4) Spring boot 환경 설정 - 완료
...
2) 블로그 : UI 구상중
3) 각 메뉴 : 마스터 테이블 형태로 메뉴를 데이터 베이스화하여 다양한 장르화 하기
