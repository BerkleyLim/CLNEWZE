...
function AdminSheetmusic() {
  ...
  const deleteMenu = (index) => {
    ...
    URI.post(process.env.REACT_APP_API_ROOT + "practiceroom/master/menu/delete", deleteData)
    .then((res) => {
      setMenuData(
        update(menuData, {
          $splice: [[index, 1]],
  ...
  const updateMenu = (data, index) => {
    URI.post(process.env.REACT_APP_API_ROOT + "practiceroom/master/menu/update", data)
    .then((res) => {
        setMenuData(
          update(menuData, {
            $merge: {[index]: data},
