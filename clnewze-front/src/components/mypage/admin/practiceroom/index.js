...
function AdminSheetmusic() {
  ...
  const addMenu = () => {
    URI.post(
      process.env.REACT_APP_API_ROOT + "/api/practiceroom/master/menu/create",
      createMenuData
      )
      .then((res) => {
        setMenuData(
  ...
  const changeOrderby = () => {
    ...
    menuData?.map((data) => {
      URI.post(
        process.env.REACT_APP_API_ROOT + "/api/practiceroom/master/menu/change/orderby",
        data
      )
        .then((res) => {
          console.log("성공!" + res);
  ...
  const deleteMenu = (index) => {
    ...
    URI.post(process.env.REACT_APP_API_ROOT + "/api/practiceroom/master/menu/delete", deleteData)
    .then((res) => {
      setMenuData(
        update(menuData, {
          $splice: [[index, 1]],
  ...
  const updateMenu = (data, index) => {
    URI.post(process.env.REACT_APP_API_ROOT + "/api/practiceroom/master/menu/update", data)
    .then((res) => {
        setMenuData(
          update(menuData, {
            $merge: {[index]: data},
