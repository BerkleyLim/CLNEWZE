...
function AdminSheetmusic() {
  ...
  const deleteMenu = (index) => {
    ...
    restApiUser.post(process.env.REACT_APP_API_ROOT + "sheetmusic/master/menu/delete", deleteData)
    .then((res) => {
      setMenuData(
        update(menuData, {
          $splice: [[index, 1]],
  ...
  const updateMenu = (data, index) => {
    restApiUser.post(process.env.REACT_APP_API_ROOT + "sheetmusic/master/menu/update", data)
    .then((res) => {
        setMenuData(
          update(menuData, {
            $merge: {[index]: data},
