...
function AdminTeacher() {
  ...
  const deleteMenu = (index) => {
    ...
    restApiAllUser.post(process.env.REACT_APP_API_ROOT + "teacher/master/menu/delete", deleteData)
    .then((res) => {
      setMenuData(
        update(menuData, {
          $splice: [[index, 1]],
  ...
  const updateMenu = (data, index) => {
    restApiAllUser.post(process.env.REACT_APP_API_ROOT + "teacher/master/menu/update", data)
    .then((res) => {
        setMenuData(
          update(menuData, {
            $merge: {[index]: data},
